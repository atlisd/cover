...
public class ApiClient : IApiClient
{
    ...
    public async Task<BalanceDto?> GetBalanceAsync()
    {
        ...
        if (response.StatusCode == HttpStatusCode.Unauthorized) { HandleUnauthorized(); return null; }
    ...
    public async Task<PagedResult<ExpenseDto>> GetExpensesAsync(int page = 1, int pageSize = 20, int? paidById = null)
    {
        ...
        if (response.StatusCode == HttpStatusCode.Unauthorized) { HandleUnauthorized(); return new PagedResult<ExpenseDto>([], 0, page, pageSize); }
    ...
    public async Task<ExpenseDto> CreateExpenseAsync(CreateExpenseRequest request)
    {
        ...
        if (response.StatusCode == HttpStatusCode.Unauthorized) { HandleUnauthorized(); throw new UnauthorizedAccessException(); }
        response.EnsureSuccessStatusCode();
        return (await response.Content.ReadFromJsonAsync<ExpenseDto>(JsonOptions))!;
    }
